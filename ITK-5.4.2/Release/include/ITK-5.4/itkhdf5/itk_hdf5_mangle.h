...
Mac: nm -g lib/libitkhdf5* 2> /dev/null | grep " [TDRS] " | awk '{ print substr($3, 2); }' | awk '{ sub(/itk_/, ""); print }' | sed 's/\(.*\)/#define \1\ itk_\1/' | sort -u > mac_hdf5_mangle.txt

The following command was used to obtain the symbol list on Linux:

Linux:
export LC_COLLATE=POSIX
nm -g lib/libitkhdf5* 2> /dev/null | grep " [TDRS] " | awk '{ print $3 }' | awk '{ sub(/itk_/, ""); print }' | sed 's/\(.*\)/#define \1\ itk_\1/' | sort -u > linux_hdf5_mangle.txt
...
export LC_COLLATE=POSIX
cat symbol_table.txt | dos2unix | grep -E "(UNDEF|SECT).*External" | grep -i "H5\w" | awk '{print $(NF) }' | awk '!/itk_/ { print }' | awk '{ if (a[$1]++ == 0) print $0; }' "$@" | sed 's \(.*\) \1\ itk_\1 ' | sed 's/^/#define /' | sort -u > win_hdf5_mangle.txt
